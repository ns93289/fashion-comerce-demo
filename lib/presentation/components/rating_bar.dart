...
class _RatingBarState extends State<RatingBar> {
  ...
  Widget _buildRating(BuildContext context, int index) {
    ...
    if (index >= _rating) {
      ...
      resolvedRatingWidget = SizedBox(width: widget.itemSize, height: widget.itemSize, child: FittedBox(child: ratingWidget?.full ?? item));
      iconRating += 1.0;
    }
...
class _HalfRatingWidget extends StatelessWidget {
  const _HalfRatingWidget({required this.size, required this.child, required this.enableMask, required this.rtlMode, required this.unratedColor});
          ...
          enableMask
              ? Stack(
                ...
                children: [
                  ...
                  FittedBox(child: ClipRect(clipper: _HalfClipper(rtlMode: rtlMode), child: Icon(Icons.star_half, color: Colors.amber))),
                ],
              )
...
class _HalfClipper extends CustomClipper<Rect> {
  ...
  @override
  Rect getClip(Size size) => rtlMode ? Rect.fromLTRB(size.width / 2, 0, size.width, size.height) : Rect.fromLTRB(0, 0, size.width / 2, size.height);
...
class _NoRatingWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      height: size,
      width: size,
      child: FittedBox(child: enableMask ? ColorFiltered(colorFilter: ColorFilter.mode(unratedColor, BlendMode.srcIn), child: child) : child),
    );
  }
}
